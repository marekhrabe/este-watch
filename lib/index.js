 ...
 *  - Windows does not watch nested directories.
 ...
 * It's trade-off, but it works and does not burn CPU.
 *
 * @param {Array.<string>} dirs
 * @param {Function} onChange Callback argument props: fileName, fileExtension.
 * @param {Object} fs
 * @param {Object} path
 * @param {Object} glob Is used only for Windows to retrieve subdirs.
 * @constructor
 */
function EsteWatch(dirs, onChange, fs, path, glob) {
  this.dirs = dirs;
...
/**
 * Restart on subdir change to track new dirs for Windows.
 * @protected
 */
...
EsteWatch.prototype.watchDirs = function() {
  this.dirs.forEach(function(dir) {
    // Windows needs to listen also subdirs.
    var suffix = this.isWindowPlatform() ? '/**/' : '';
    var path = this.path.join(dir, suffix);
...
EsteWatch.prototype.onDirChange = function(dir, action, watchPath) {
  ...
  // Don't propagate deletes files on non Win platforms too.
  // Also, fs.statSync fails on deleted symlink dir with "Abort trap: 6".
  // https://github.com/bevry/watchr/issues/42
  // https://github.com/joyent/node/issues/4261
  if (!pathExists) return;

  if (this.isWindowPlatform() && this.fs.statSync(path).isDirectory()) {
    // Restart watching to track new dirs for Windows.
 ...
 * @protected
 */
EsteWatch.prototype.isWindowPlatform = function() {
